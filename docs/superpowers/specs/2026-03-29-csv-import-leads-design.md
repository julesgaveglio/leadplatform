...
| Synonymes CSV | Champ Lead |
|---|---|
| nom, company, société, name, entreprise, raison sociale | `company_name` |
...
| proprietaire, owner, gerant, gérant | `owner_name` |
| score | `score` |
...
4. Bouton "Importer X leads" → insert via `supabase.from('leads').insert([...])` avec `Promise.allSettled`
5. Valeurs par défaut injectées à l'insert : `status: 'to_call'`, `score: 0`, `scoring_status: 'partial'`, `demo_status: 'idle'`, `google_reviews_count: 0`, `country: 'fr'` (sauf si mappé)
...
Skip silencieux : un lead est considéré doublon si `company_name` (normalisé lowercase + trim) **et** `city` (normalisé) correspondent à un lead existant. Les doublons sont comptabilisés dans le badge résumé mais jamais insérés.
...
- Échec d'insert partiel → toast avec nombre de lignes échouées
...
| `app/(dashboard)/leads/page.tsx` | Modifier — ajouter onglet Import |
| `package.json` | Modifier — ajouter `papaparse` |
